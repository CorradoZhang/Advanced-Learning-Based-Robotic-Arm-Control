...
def run(
    ...
    if env_name == "robosuite":
        from panda_cube_grasp import RobosuitePandaCubeGraspEnv

        env = RobosuitePandaCubeGraspEnv(render_mode=render_mode)
    else:
    ...
    try:
        ...
        for _ in range(steps):
            if policy == "scripted":
                if env_name == "robosuite":
                    relative_offset = info["cube_position"] - info["hand_position"]
                    gripper = -1.0 if env.time > 4.0 else 1.0
                    action = np.array(
                        [
                            np.clip(relative_offset[0] / 0.05, -1.0, 1.0),
                            np.clip(relative_offset[1] / 0.05, -1.0, 1.0),
                            np.clip((relative_offset[2] + 0.03) / 0.05, -1.0, 1.0),
                            0.0,
                            0.0,
                            0.0,
                            gripper,
...
def main() -> None:
    ...
    parser.add_argument("--env", choices=("local", "robosuite"), default="local")
    ...
    run(env_name=args.env, policy=args.policy, steps=args.steps, render_mode=render_mode)
