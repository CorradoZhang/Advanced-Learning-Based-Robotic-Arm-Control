...
- PandaCubeGraspMocapEnv:
  Mocap-weld control. External code writes an absolute mocap target pose, and MuJoCo
  constraints move the arm toward that target while the gripper remains actuator-driven.
- PandaCubeGraspOSCPositionEnv:
  robosuite-style OSC_POSITION control. The policy outputs xyz end-effector deltas plus
  one gripper command, and an operational-space controller writes joint torques.
...
OSC_POSITION_SCALE = np.array([0.05, 0.05, 0.05], dtype=np.float64)
OSC_POSITION_KP = np.array([800.0, 800.0, 800.0], dtype=np.float64)
OSC_ORIENTATION_KP = np.array([150.0, 150.0, 150.0], dtype=np.float64)
OSC_DAMPING_RATIO = 1.0
OSC_NULLSPACE_KP = 10.0
OSC_MAX_GOAL_DELTA = np.array([0.20, 0.20, 0.20], dtype=np.float64)
...
MOCAP_BODY_NAME = "hand_mocap_target"
MOCAP_WELD_NAME = "hand_mocap_weld"
...
def _nullspace_torques(
    ...
    joint_kp: float = OSC_NULLSPACE_KP,
) -> np.ndarray:
...
def _opspace_matrices(
    ...
    lambda_full = np.linalg.pinv(jac_full @ mass_matrix_inv @ jac_full.T)
    lambda_pos = np.linalg.pinv(jac_pos @ mass_matrix_inv @ jac_pos.T)
    lambda_ori = np.linalg.pinv(jac_ori @ mass_matrix_inv @ jac_ori.T)
    jac_bar = mass_matrix_inv @ jac_full.T @ lambda_full
    nullspace_matrix = np.eye(jac_full.shape[1], dtype=np.float64) - jac_bar @ jac_full
...
class PandaCubeGraspEnv(gym.Env[np.ndarray, np.ndarray]):
    ...
    def __init__(
        ...
        self._mocap_body_id = _named_id(self.model, mujoco.mjtObj.mjOBJ_BODY, MOCAP_BODY_NAME)
        self._mocap_id = int(self.model.body_mocapid[self._mocap_body_id])
        if self._mocap_id < 0:
            raise ValueError(f"Body is not configured as a MuJoCo mocap body: {MOCAP_BODY_NAME}")
        self._mocap_weld_id = _named_id(self.model, mujoco.mjtObj.mjOBJ_EQUALITY, MOCAP_WELD_NAME)
        ...
        self._arm_actuator_ids = list(range(7))
        self._arm_actuator_kp = self.model.actuator_gainprm[self._arm_actuator_ids, 0].copy()
        self._arm_actuator_kv = -self.model.actuator_biasprm[self._arm_actuator_ids, 2].copy()
    ...
    def _reset_simulation_state(self) -> None:
        ...
        self.data.eq_active[self._mocap_weld_id] = 0
        self._ctrl_target = HOME_CTRL.copy()
        self.data.ctrl[:] = self._ctrl_target
        self._reset_cube_pose()
        mujoco.mj_forward(self.model, self.data)
        self._sync_mocap_to_hand()

    def _sync_mocap_to_hand(self) -> None:
        self.data.mocap_pos[self._mocap_id] = self.data.xpos[self._hand_body_id]
        self.data.mocap_quat[self._mocap_id] = _mat_to_quat(self.data.xmat[self._hand_body_id])

    def _zero_arm_actuator_force_targets(self) -> np.ndarray:
        qpos = self.data.qpos[self._arm_qpos_adrs]
        qvel = self.data.qvel[self._arm_dof_adrs]
        neutral_ctrl = qpos + (self._arm_actuator_kv / self._arm_actuator_kp) * qvel
        return np.clip(neutral_ctrl, self._ctrl_low[:7], self._ctrl_high[:7])
...
class PandaCubeGraspMocapEnv(PandaCubeGraspEnv):
    """Panda grasp environment controlled by a mocap target welded to the hand body."""
    ...
    def __init__(
        ...
        # External callers set the 6-DoF target pose directly. The action only controls gripper opening.
        self.action_space = spaces.Box(low=-1.0, high=1.0, shape=(1,), dtype=np.float32)
        self.data.eq_active[self._mocap_weld_id] = 1
        self._sync_mocap_to_hand()
        mujoco.mj_forward(self.model, self.data)
    ...
    def set_mocap_pose(self, position: np.ndarray, quat: np.ndarray) -> None:
        quat = np.asarray(quat, dtype=np.float64)
        quat_norm = np.linalg.norm(quat)
        if quat_norm <= 0.0:
            raise ValueError("Mocap quaternion must have non-zero norm.")

        self.data.mocap_pos[self._mocap_id] = np.asarray(position, dtype=np.float64)
        self.data.mocap_quat[self._mocap_id] = quat / quat_norm

    def set_mocap_pose_from_matrix(self, position: np.ndarray, rotation: np.ndarray) -> None:
        self.set_mocap_pose(position, _mat_to_quat(rotation))
    ...
    def step(self, action: np.ndarray) -> tuple[np.ndarray, float, bool, bool, dict]:
        ...
        self._ctrl_target[7] = np.clip(
            self._ctrl_target[7] + action[0] * IK_GRIPPER_SCALE,
            self._ctrl_low[7],
            self._ctrl_high[7],
        )

        for _ in range(self.frame_skip):
            # Cancel the arm actuator's affine spring-damper force so the mocap weld drives the arm.
            self._ctrl_target[:7] = self._zero_arm_actuator_force_targets()
            self.data.ctrl[:] = self._ctrl_target
            mujoco.mj_step(self.model, self.data)
...
class PandaCubeGraspOSCPositionEnv(PandaCubeGraspEnv):
    """Panda grasp environment with a robosuite-style OSC_POSITION torque controller."""
    ...
    def __init__(
        ...
        self._osc_position_scale = OSC_POSITION_SCALE.copy()
        self._osc_kp = np.concatenate([OSC_POSITION_KP, OSC_ORIENTATION_KP])
        self._osc_kd = 2.0 * np.sqrt(self._osc_kp) * OSC_DAMPING_RATIO
        self._osc_goal_pos: np.ndarray | None = None
        self._osc_goal_rot: np.ndarray | None = None
        self._osc_initial_joint = HOME_ARM.copy()
        self._arm_force_low = self.model.actuator_forcerange[:7, 0].copy()
        self._arm_force_high = self.model.actuator_forcerange[:7, 1].copy()
        self._reset_osc_targets()
    ...
    def set_osc_position_target(self, position: np.ndarray) -> None:
        self._osc_goal_pos = np.asarray(position, dtype=np.float64)

    def _reset_osc_targets(self) -> None:
        self.data.eq_active[self._mocap_weld_id] = 0
        self._osc_goal_pos = self.data.xpos[self._hand_body_id].copy()
        self._osc_goal_rot = self.data.xmat[self._hand_body_id].reshape(3, 3).copy()
        self._osc_initial_joint = self.data.qpos[self._arm_qpos_adrs].copy()
        self.data.qfrc_applied[:] = 0.0

    def _arm_mass_matrix(self) -> np.ndarray:
        mass_matrix = np.empty((self.model.nv, self.model.nv), dtype=np.float64)
        mujoco.mj_fullM(self.model, mass_matrix, self.data.qM)
        return mass_matrix[np.ix_(self._arm_dof_adrs, self._arm_dof_adrs)]

    def _hand_jacobians(self) -> tuple[np.ndarray, np.ndarray]:
        jac_pos = np.zeros((3, self.model.nv), dtype=np.float64)
        jac_ori = np.zeros((3, self.model.nv), dtype=np.float64)
        mujoco.mj_jacBody(self.model, self.data, jac_pos, jac_ori, self._hand_body_id)
        return jac_pos[:, self._arm_dof_adrs], jac_ori[:, self._arm_dof_adrs]

    def _osc_torques(self) -> np.ndarray:
        if self._osc_goal_pos is None or self._osc_goal_rot is None:
            self._reset_osc_targets()

        joint_pos = self.data.qpos[self._arm_qpos_adrs].copy()
        joint_vel = self.data.qvel[self._arm_dof_adrs].copy()
        hand_pos = self.data.xpos[self._hand_body_id].copy()
        hand_rot = self.data.xmat[self._hand_body_id].reshape(3, 3).copy()

        jac_pos, jac_ori = self._hand_jacobians()
        jac_full = np.vstack([jac_pos, jac_ori])
        hand_pos_vel = jac_pos @ joint_vel
        hand_ori_vel = jac_ori @ joint_vel

        position_error = self._osc_goal_pos - hand_pos
        orientation_error = _rotation_error_world(hand_rot, self._osc_goal_rot)
        desired_force = self._osc_kp[:3] * position_error - self._osc_kd[:3] * hand_pos_vel
        desired_torque = self._osc_kp[3:] * orientation_error - self._osc_kd[3:] * hand_ori_vel

        mass_matrix = self._arm_mass_matrix()
        _, lambda_pos, lambda_ori, nullspace_matrix = _opspace_matrices(mass_matrix, jac_full, jac_pos, jac_ori)
        decoupled_wrench = np.concatenate([lambda_pos @ desired_force, lambda_ori @ desired_torque])
        torques = jac_full.T @ decoupled_wrench
        torques += self.data.qfrc_bias[self._arm_dof_adrs]
        torques += _nullspace_torques(
            mass_matrix,
            nullspace_matrix,
            self._osc_initial_joint,
            joint_pos,
            joint_vel,
        )
        return np.clip(torques, self._arm_force_low, self._arm_force_high)

    def step(self, action: np.ndarray) -> tuple[np.ndarray, float, bool, bool, dict]:
        action = np.asarray(action, dtype=np.float64)
        action = np.clip(action, self.action_space.low, self.action_space.high)

        if self._osc_goal_pos is None:
            self._reset_osc_targets()
        goal_delta = action[:3] * self._osc_position_scale
        self._osc_goal_pos = np.clip(
            self.data.xpos[self._hand_body_id] + goal_delta,
            self.data.xpos[self._hand_body_id] - OSC_MAX_GOAL_DELTA,
            self.data.xpos[self._hand_body_id] + OSC_MAX_GOAL_DELTA,
        )
        self._ctrl_target[7] = np.clip(
            self._ctrl_target[7] + action[3] * IK_GRIPPER_SCALE,
            self._ctrl_low[7],
            self._ctrl_high[7],
        )
        ...
        for _ in range(self.frame_skip):
            ...
            self._ctrl_target[:7] = self._zero_arm_actuator_force_targets()
            self.data.ctrl[:] = self._ctrl_target
            self.data.qfrc_applied[self._arm_dof_adrs] = self._osc_torques()
            mujoco.mj_step(self.model, self.data)
